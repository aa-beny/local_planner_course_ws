...
class Config:
    ...
    def __init__(self):
        ...
        self.max_yaw_rate = 40.0 * math.pi / 180.0  # [rad/s] 最大角速度 ω，每秒幾度再轉弧度
        ...
        # speed_cost：
        # 因為 max_speed = 1.0，所以原始值大概在 0 到 1 之間。
...
def calc_control_and_trajectory(x, dw, config, goal, ob):
    ...
    # 保留目前狀態，所有候選軌跡都從同一個狀態開始模擬，x[:]是複製的意義
    x_init = x[:]
...
def calc_obstacle_cost(trajectory, ob, config):
    ...
    if config.robot_type == RobotType.rectangle:
        ...
        # 例如地圖上的 (3, 5)。
        # 但是矩形機器人會旋轉，
        # 所以程式要把障礙物轉成「以機器人為中心」的座標。轉換後就可以判斷障礙物有沒有落在車體矩形範圍內。
...
def calc_to_goal_cost(trajectory, goal):
    ...
    # 目標方向角
    # atan2(dy, dx) 會把向量 (dx, dy) 轉成方向角。
    # 也就是：從軌跡最後一點看過去，目標位在哪個方向。
    ...
    # 把角度差限制在 [-pi, pi]，再取絕對值
    # 角度是循環的，例如 179° 和 -179° 其實只差 2°，
    # 不能直接用相減結果判斷。
    # atan2(sin(angle_error), cos(angle_error))
    # 可以把角度差限制在 [-pi, pi]，也就是 [-180°, 180°]。
    # 最後用 abs() 取絕對值，因為 cost 只在意偏差大小，不在意往左偏或往右偏。
