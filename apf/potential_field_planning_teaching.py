...
# STEP_GAIN：把總合力 F_sum 轉成這一次要移動多少的比例。
# 這是這份模擬程式用來更新位置的設定，不是 APF 理論公式裡一定會出現的參數。
#     move_step = STEP_GAIN * F_sum
#
# 所以：
# - F_sum 的方向會決定機器人往哪裡走。
# - F_sum 的大小會影響這一步走多遠。
# - STEP_GAIN 越大，每一步走得越明顯，但太大可能會讓路徑看起來比較跳。
STEP_GAIN = 0.001
...
# 使用方式：
# 把下面其中一組數值複製到上面的 APF 參數設定，
# 然後重新執行程式，觀察路徑、力的方向和最後結果有什麼差別。


# 1. 正常成功：
# 機器人會提早被障礙物推開，繞過障礙物後到達目標。
# ATTRACTIVE_GAIN_ETA = 10.0
# REPULSIVE_GAIN_K = 200.0
# SAFE_DISTANCE_D0 = 4.0
...
# 2. 引力太強、斥力太弱：
# 機器人會很想往目標直線衝，障礙物推不太動它。
# 可以觀察它是不是太晚轉彎，甚至太靠近障礙物。
...
# 4. 斥力太強、d0 太大：
# 障礙物很遠就開始推機器人，而且推得很大力。
# 可以觀察機器人是不是被推得太遠，最後反而走不到目標。
